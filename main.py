...
# I'll probably need to tune these after testing with real movements
...
class FallDetector:
    ...
    def __init__(self):
        ...
        self._phase_timeout = 2.0  # seconds
    ...
    def update(self, reading: dict) -> dict | None:
        ...
        if self._state == 'MONITORING':
            ...
            # if acceleration drops below the freefall threshold, move to next phase
        ...
        # freefall phase - waiting for an impact spike
        ...
        # impact happened - now waiting to see if person stays still
            ...
            if STILLNESS_MIN <= mag <= STILLNESS_MAX:
                if self._stillness_start_time is None:
                    self._stillness_start_time = now  # start the stillness timer
                elif now - self._stillness_start_time >= STILLNESS_DURATION:
                    # they've been still long enough - this is a confirmed fall
            ...
            # safety timeout - if we've been stuck in impact phase too long, reset
